...
Box_2d::Box_2d(const Vec2d &center, const double heading, const double length,
    ...
    :
     center_(center),
      ...
      { float jiao=atan(half_width()/(half_length()+0.01));
      point corner1, corner2, corner3, corner4;
     corner1.x=center_x()+ half_diagonal()* cos(heading-jiao);
     corner1.y=center_y()+ half_diagonal()* sin(heading- jiao );
     Box2d_corner.push_back(corner1); 
     corner2.x=center_x()+ half_diagonal()* cos(heading +jiao );
     corner2.y=center_y()+ half_diagonal()* sin(heading+ jiao );
     Box2d_corner.push_back(corner2);
     Box2d_corner.push_back(corner2);
     corner3.x=center_x()+ half_diagonal()* cos(heading- jiao +M_PI);
     corner3.y=center_y()+ half_diagonal()* sin(heading- jiao +M_PI);
     Box2d_corner.push_back(corner3);
      Box2d_corner.push_back(corner3);
     corner4.x=center_x()+ half_diagonal()* cos(heading+jiao +M_PI);
     corner4.y=center_y()+ half_diagonal()* sin(heading+ jiao +M_PI);
