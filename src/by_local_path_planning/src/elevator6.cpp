...
void erweima_class::erweimaGetCallBack_erweima(const apriltag_ros::AprilTagDetectionArray& msg){
  ...
  if(msg.detections.size()>0)
  {
    ...
    {
       if(msg.detections[i].id[0]==26)
       {
          view_ma = 1;
         tf::Quaternion q;
         tf::quaternionMsgToTF(msg.detections[i].pose.pose.pose.orientation, q);
         tf::Matrix3x3(q).getRPY(raw, pitch, theta);
         control_erweima_y2 = msg.detections[i].pose.pose.pose.position.x-0.12 ;//若左偏就加0.06
       ...
       if(msg.detections[i].id[0]== 11)
       {
        ...
        if(msg.detections[i].id[0]==13)
       {
         view_ma_13 = 1;
         tf::Quaternion q;
         tf::quaternionMsgToTF(msg.detections[i].pose.pose.pose.orientation, q);
         tf::Matrix3x3(q).getRPY(raw, pitch, theta);
         dis13 = sqrt(pow(msg.detections[i].pose.pose.pose.position.x, 2) + pow(msg.detections[i].pose.pose.pose.position.z, 2));
         cout<<"dis13::::::"<<dis13<<endl;
       }
    }
  ...
  cout<<"dis-----zailimian"<<dis<<endl;
    ...
    }else if(over1==1 && task == 7 && acc_in == 1){//第二个二维码
...
void erweima_class::nodeStart(int argc, char  *argv[]){
    ...
    while (ros::ok)
    {
      ...
      if(task == 6 || task == 7 || task == 8)
      {
        ...
        if(dis2 < 0.6 && view_ma == 0 && view_ma_13 == 0 && dis13 < 0.3){
            cmd_vel.linear.y=0;
            cmd_vel.linear.x=0;
            cmd_vel.angular.z=0;
            pub_control_cmd_erweima.publish(cmd_vel);
        ...
        if(dis2 < 0.6 && view_ma == 0 && view_ma_13 == 0){
            cmd_vel.linear.y=0;
            cmd_vel.linear.x=0;
            cmd_vel.angular.z=0;
            pub_control_cmd_erweima.publish(cmd_vel);
