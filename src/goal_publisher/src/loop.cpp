...
{
public:
    GoalPublisher() : goal_(0), last_goal_(0), goal1_loaded_(false), goal2_loaded_(false), goal0_loaded_(false),
                      loop_count_(0), max_loops_(0), current_goal_(0), start_goal_(0), count_on_arrival_(false)
    {
        nh_ = ros::NodeHandle();
        int odom_ = -1;
        int slam_ = -1;
        ...
        while (ros::ok())
        {
            ...
            if (current_task == 1 && goal_ != last_goal_)
            {
                if (goal_ == 1 && goal1_loaded_)
                {
                    ...
                    publishGoal(goal1_);
                    ROS_INFO("Publishing the FIRST goal");
                    current_goal_ = 1;
                    start_goal_ = 1;
                }
                else if (goal_ == 2 && goal2_loaded_)
                {
                    ...
                    ros::Duration(1).sleep();
                    publishGoal(goal2_);
                    ROS_INFO("Publishing the SECOND goal");
                    current_goal_ = 2;
                    start_goal_ = 2;
                }
                last_goal_ = goal_;
        ...
        if (SLAM)
        {
                     ...
                     pow(vehicleState.y - goal1_.pose.position.y, 2)) < 0.5)
            {
                if (!goal0_loaded_ && current_goal_ == 1)
                {
                    ...
                    if (count_on_arrival_ && start_goal_ == 1)
                    {
                        loop_count_++;
                          ...
                          pow(vehicleState.y - goal2_.pose.position.y, 2)) < 0.5)
            {
                if (!goal0_loaded_ && current_goal_ == 2)
                {
                    ...
                    goal0_.header.frame_id = "map";
                    goal0_loaded_ = true;
                    ROS_INFO("Reached goal2, setting next goal to goal1");
                    if (goal0_id_ == start_goal_)
                    {
                        count_on_arrival_ = true;
                    }
                    if (count_on_arrival_ && start_goal_ == 2)
                    {
                        loop_count_++;
                        count_on_arrival_ = false;
                        if (max_loops_ == -1)
